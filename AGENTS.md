...
| **Encoder-Driven Hypercar Scene** | `MockWheelTelemetryService` feeds measured-motion-shaped left/right samples to `EncoderDriveViewModel`, which owns the 5% straight and 20% gentle/strong response bands plus every finite vehicle-pose and continuous-road path value. Passive `EncoderDriveView`/`HypercarView` render one road with no lane divider and a vector rear-view hypercar. Future hardware must replace the mock with measured encoder telemetry through the same boundary; commanded PWM is never encoder feedback or odometry. |
