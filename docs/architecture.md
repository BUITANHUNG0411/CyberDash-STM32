...
> **AI Context**: Active specification of ownership and data flow between the C++17 services, focused context ViewModels, and passive QML view.

## 1. Ownership and Lifetime

`main.cpp` owns all application services and the retained ViewModels exposed through the `QQmlContext`. They remain alive for the duration of `app.exec()`.
...
| `ThemeController` | `ThemeViewModel` | Day/night theme |
...
| `CenterHubController` | `CenterHubViewModel` | C++-owned Music and Parking Assist page selection |
...
page `1`; clear, caution, stale, and unavailable samples leave the requested non-critical page
selected unless the user changes it. `CenterHub.qml` keeps a stable `StackLayout` and forwards
its horizontal `DragHandler` lifecycle to C++; an 80 px left/right threshold preserves Music/Parking
quick navigation. A live critical sample always overrides a manual Music request. Both ViewModels
and the mock timer remain GUI-thread objects. A future STM32 adapter
...
The active application graph is intentionally limited to telemetry, the Car dashboard, Music,
Parking Assist, Trip Computer, and day/night theme concerns.
...
QML is a passive declarative view. Imperative functions, control flow, local mutation, and block-form event handlers are forbidden. Property bindings, ternary presentation bindings, and one direct `Q_INVOKABLE` call from a handler are allowed.
